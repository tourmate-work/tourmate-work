...
function VehicleVectorGraphic({ type }: { type: string }) {
  if (type === "sport") {
    return (
      <svg
        viewBox="0 0 450 160"
        className="w-full max-w-[360px] h-auto text-slate-300 drop-shadow-sm"
        fill="currentColor"
      >
...
export function VehiclesCatalog() {
  const [selectedVehicle, setSelectedVehicle] = useState<VehicleDetail>(VEHICLES[0]);
  ...
  // Other cars list (filtering out currently selected one or showing the rest)
                ...
                <button
                  ...
                  className={`relative h-20 w-24 sm:h-24 sm:w-28 rounded-2xl overflow-hidden border-2 transition-all ${
                    activeThumbnailIndex === idx
                      ? "border-violet-600 ring-2 ring-violet-600/30 scale-105"
                      : "border-slate-200 opacity-70 hover:opacity-100"
                  }`}
                >
                  <Image
                    src={thumb}
                    alt={`${selectedVehicle.name} photo preview ${idx + 1}`}
                    fill
        ...
        <div className="pt-10 border-t border-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {otherCars.map((car) => (
              ...
              >
                {/* Silhouette / Car graphic container */}
                ...
                <div className="space-y-4">
                  ...
                  <button
                    ...
                    className="w-full bg-violet-600 hover:bg-violet-700 text-white font-semibold text-sm py-3 rounded-2xl shadow-sm hover:shadow-md transition-all active:scale-[0.98] text-center"
                  >
                    View Details
                  </button>
                </div>
