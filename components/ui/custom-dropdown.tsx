...
export interface CustomDropdownProps {
  ...
  placeholder?: string;
  variant?: "light" | "purple" | "green" | "seller" | "default";
...
export function CustomDropdown({
  ...
  if (isGreen) {
    triggerStyle += ` bg-emerald-700/80 hover:bg-emerald-700 border-emerald-500/60 text-white focus:ring-2 focus:ring-white/40 ${
      isOpen ? "ring-2 ring-white/40 shadow-lg" : ""
    }`;
  } else if (isLight) {
    triggerStyle += ` bg-slate-100/90 hover:bg-slate-100 dark:bg-white/10 dark:hover:bg-white/15 border-slate-200 dark:border-white/10 text-slate-800 dark:text-white focus:ring-2 focus:ring-emerald-500 ${
      isOpen ? "ring-2 ring-emerald-500 border-emerald-500 shadow-md" : ""
    }`;
  } else {
    // Seller & Default style
    triggerStyle += ` bg-slate-50 hover:bg-slate-100/80 dark:bg-[#15151a] dark:hover:bg-[#1c1c24] border-slate-200 dark:border-white/10 text-slate-900 dark:text-white focus:ring-2 focus:ring-emerald-500 ${
      isOpen ? "ring-2 ring-emerald-500/40 border-emerald-500 dark:border-emerald-400 shadow-md" : ""
    }`;
  }
      ...
      {isOpen && (
        <div
          ...
          className={`absolute left-0 right-0 z-50 rounded-2xl shadow-2xl border overflow-hidden p-1.5 max-h-64 overflow-y-auto backdrop-blur-xl transition-all duration-200 ${
            ...
            isGreen
              ? "bg-emerald-950/95 border-emerald-700/70 text-white"
            ...
            return (
              <button
                ...
                className={`w-full flex items-center justify-between px-3.5 py-2.5 text-left text-xs sm:text-sm rounded-xl transition-all duration-150 my-0.5 group cursor-pointer ${
                  isSelected
                    ...
                    : isGreen
                    ? "text-emerald-200 hover:bg-emerald-900/60 hover:text-white font-medium"
              ...
              >
                ...
                <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                  {opt.badge && (
                    <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-700/40">
                      {opt.badge}
                    </span>
                  )}
                  {isSelected && (
                    <Check
                      className={`h-4 w-4 animate-in zoom-in-75 duration-150 ${
                        isGreen ? "text-white" : "text-emerald-600 dark:text-emerald-400"
                      }`}
                    />
                  )}
                </div>
